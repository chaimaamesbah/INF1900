...
void Roue::avancer(uint8_t vitessePourcentage) {
    ...
    definirVitesse(DROITE, AVANCER, vitessePourcentage + 10);
}

void Roue::reculer(uint8_t vitessePourcentage) {
    definirVitesse(GAUCHE, RECULER, vitessePourcentage);
    definirVitesse(DROITE, RECULER, vitessePourcentage + 10);
}

// Tourner
void Roue::tourner(uint8_t vitessePourcentageGauche, uint8_t vitessePourcentageDroite) {
    definirVitesse(GAUCHE, AVANCER, vitessePourcentageGauche); // si tourner gauche : plus basse
    definirVitesse(DROITE, AVANCER, vitessePourcentageDroite); // si tourner droite : plus basse
}
...
void Roue::tournerSurPlaceDroite(uint8_t vitessePourcentage) {
    definirVitesse(GAUCHE, AVANCER, vitessePourcentage);
    definirVitesse(DROITE, RECULER, vitessePourcentage + 5);
}

void Roue::tournerSurPlaceGauche(uint8_t vitessePourcentage) {
    definirVitesse(DROITE, AVANCER, vitessePourcentage);
    definirVitesse(GAUCHE, RECULER, vitessePourcentage + 5);
}
